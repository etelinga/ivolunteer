...
public class TEA {
    ...
    public static void main(String args[]) {
        ...
        int enc[] = t.encode(plainSource, plainSource.length);
        String encodedString = t.toString(enc);
        System.out.println("Encoded String: <" + encodedString + ">");
        int enc2[] = t.toInts(encodedString);
        byte dec2[] = t.decode(enc2);
        System.out.println("output dec2= " + new String(dec2));
        ...
        System.out.println("output dec= " + new String(dec));
    ...
    /**
     * Default constructor chooses a Key for discoverant
     */
    public TEA() {
        this(new BigInteger("39e859f86df9b909a8c87cb8d9bd599", 16).toByteArray());
    }
    ...
    /**
     * Create a String for password file and serialization
     */
    public String toString(int[] enc) {
    ...
    /**
     * From a String, re-create an array of int's for use by
     * decode
     */
    public int[] toInts(String s) {

        StringTokenizer intST = new StringTokenizer(s, "l");
        int nbints = intST.countTokens();
        int[] ints = new int[nbints];

        int i = 0;

        while (intST.hasMoreTokens()) {
            String res = intST.nextToken();
            ints[i] = Integer.parseInt(res);
            i++;
        }
        return ints;
    }
    ...
    public byte[] decode(byte b[], int count) {
        ...
        try {
            ...
            // If we have any problems, the guy probably wasn't encoded
        }
    ...
    /**
     * This method is helper method for use in encoding the supplied
     * String.
     *
    ...
    public static String encode(String password) {
        ...
        TEA tea = new TEA();  // instantiate using the default key
        byte[] pp = password.getBytes(); // plain password
        int[] enc = tea.encode(pp, pp.length);
        return tea.toString(enc);   // encoded password String
    }

    /**
     * This method is helper method for use in decoding the supplied
     * encrypted String.
     *
     * @param String the encoded string that is to be decoded
     * @return the decoded string
     */
    public static String decode(String encPassword) {
        if (encPassword == null || !TEA.isEncoded(encPassword))
            return encPassword;
        TEA tea = new TEA();  // instantiate using the default key
        String decoded = encPassword;
        try {
            int enc[] = tea.toInts(encPassword);
            byte dec[] = tea.decode(enc);
            decoded = new String(dec);
        } catch (Exception e) {
            // if we have a problem, then the incoming guy was not encoded
            //System.out.println("[DEBUG] TEA::decode() "+new String(decoded));
        }

        return decoded;   // encoded password String
    }

    public static boolean isEncoded(String test) {
        return true;
    }
